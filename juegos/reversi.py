...
class Reversi(TwoPlayerGame):
    ...
    def __init__(
        ...
        height: int,
        width: int,
        ignore_block_cells_in_captures: bool = False,
    ) -> None:
        super().__init__(
            "Reversi",
            player1,
            ...
            for col in range(0, self.width + 1):
                ...
                if col > 0 and row > 0:  # Actual buttons
                    if (col, row) in board:  # Black and white
                        cur_bg = ""
                        if board.get((col, row)) == self.player1.label:
                            cur_bg = "black"
                        elif board.get((col, row)) == self.player2.label:
                            cur_bg = "white"
                        else:
                            cur_bg = "grey"
                        piece = Button(gui_frame, bg=cur_bg, state=DISABLED)
                    else:  # Background
        ...
        for row in range(1, self.height + 1):
            for col in range(1, self.width + 1):
                ...
                if pos in board:  # Black and white
                    color = board.get(pos)
                    cur_bg = ""
                    if color == self.player1.label:
                        cur_bg = "black"
                    elif color == self.player2.label:
                        cur_bg = "white"
                    else:
                        cur_bg = "grey"
